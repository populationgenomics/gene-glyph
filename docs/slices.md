...
### Slice 34 — User-supplied variants from the URL

**Motivation:**
The embed view today renders curated ClinVar calls and nothing else.
A clinician looking at HBB wants to drop *their* patient's variant
onto the figure to see where it sits relative to the curated calls
— without scrolling through gnomAD, without picking a different
transcript, without leaving the page. The minimal version of that
workflow is a URL parameter: `?transcript=ENST…&variants=11:5226796C>G,…`
and a track that renders the parsed variants alongside the existing
ones. Everything fancier (HGVS, interactive entry, automatic
transcript inference) builds on this slice's parser + track.

The work splits cleanly into a tracer-bullet stack — this slice
ships only the URL → bare-track path. Slices 35-37 add selection
drop-lines, VariantValidator integration for HGVS, and the
interactive entry modal.

**In scope:**
- New `?variants=v1,v2,…` query parameter on the embed view. Coexists
  with the existing `?transcript=`; parsed at the same point in
  `readUrlParams()` and round-tripped via `history.replaceState`
- Local parser accepting the common genomic forms — `chr:pos REF>ALT`,
  `chr-pos-REF-ALT`, `chr:pos-REF-ALT`, with optional `chr` prefix,
  lowercase REF/ALT, leading / trailing whitespace, and the
  occasional `chr1:12345C >G` paste with an internal space. No
  REF/ALT character validation locally — accept whatever's between
  the separators (downstream rendering already tolerates anything
  string-shaped)
- Canonical internal form: gnomAD-style `chr-pos-REF-ALT` so user
  variants and ClinVar share an ID space. The local parser's job
  is "normalise to this string"
- New `userVariantTrack({ source, encoding? })` factory in
  `packages/gene-glyph/src/tracks/user-variant-track.tsx`. Reuses
  the entire ClinVar rendering pipeline (anchored marker + horizontal
  span line + chevron stub) via a thin adapter — same
  `placeClinVarRecords` projection, same multi-bp handling, same
  intron-flank-aware truncation, same negative-strand correctness.
  No new geometry code
- Glyph: `cross` shape, deep-purple fill
  (`--vv-user-variant-color: var(..., #7c3aed)`). Single category
  for MVP — no significance / star encoding because user variants
  don't carry it
- Track placement in the embed: between `exon` and `interpro` so
  the variants sit anchored to gene structure rather than buried
  underneath ClinVar
- No toolbar toggle — the track is shown iff `?variants=` is
  non-empty. Empty URL = no track row at all (zero-height layout
  contribution, the row disappears entirely)
- Unparseable entries don't block the figure. Collected into a
  footer status note: "2 variants couldn't be parsed: `9:abc`,
  `1:12345X`". The valid variants render
- Variants whose chr doesn't match the transcript's chr, or whose
  pos falls outside any exon (with intron-flank-aware tolerance,
  same as ClinVar), fall to the existing "Unplaced" bucket in the
  embed's left-gutter list

**Out of scope (covered by later slices):**
- HGVS forms (`c.93G>A`, `p.Val123Met`) — Slice 36 routes these
  through VariantValidator
- Interactive entry / modal — Slice 37
- Selection drop-lines + full-figure-height range overlay — Slice 35
- Variant-only URLs (no `?transcript=`) — v2; the embed surfaces a
  clear "missing transcript" error if `?variants=` is present
  without `?transcript=`
- Codon-impact / HGVS round-trip in the tooltip — v2 fanciness

**Definition of done:**
- `?transcript=ENST00000269305&variants=17:7674212C>T,17-7674212-C-A`
  renders two purple crosses on TP53 at codon 175-ish
- The same URL with `,9:bogus` appended still renders the two valid
  variants and surfaces the parse error in a footer
- Toggling between view modes (genome / transcript / protein) keeps
  the user-variants track visible, with the same truncation /
  intron-collapse behaviour ClinVar gets in Slice 31 / earlier
- Unit tests on the parser: each accepted format normalises to the
  canonical `chr-pos-REF-ALT`; each rejected form returns a parse
  error with the offending substring; whitespace / case tolerance
  pinned
- Playwright spec walks: load URL → assert track present → assert N
  glyphs → mode-switch → assert glyphs still present → invalid
  variant appended → assert footer error
...
**Motivation:**
Once user variants are on the figure, the next workflow is *compare
this variant with the curated calls underneath it*. The existing
ClinVar selection ring tells you which mark you clicked, but doesn't
help you read off "what codon, what InterPro domain, what other
variants live at the same x". A vertical drop-line through the
whole figure — anchored on the variant's selection ring at the top,
ending at the bottom of the last track — turns the figure into a
ruler you can sight along.

Multi-bp variants need *two* drop-lines (one at each end of the
horizontal span) with a lighter alpha-fill rect between them — the
existing `vv-brush-rect` styling drops straight in. SNVs collapse
to a single line (start == end). The same treatment applied to
ClinVar selection closes the comparison loop in both directions.

**In scope:**
- Selection state on the user-variants track (mirroring the existing
  ClinVar `selectedFeatureIds` flow). Single-selection for MVP —
  click toggles, click-elsewhere deselects
- `?selected=<id>` round-trips for user variants. ID encoding: FNV-1a
  32-bit hash (8 hex chars) of the canonical `chr-pos-REF-ALT`. Same
  encoding applied to ClinVar — existing ClinVar share-links keep
  working because the URL→state path tries the raw value first, then
  falls back to hashing it and looking up the result. New selections
  always emit the hash form
- Build the `Map<hash, canonicalId>` lookup at placement time (one
  pass over the placed variants on the figure, both ClinVar and
  user). Cost is sub-millisecond per page load
- New viewer overlay: an array of `{anchorX, farX, fill, stroke}`
  selection-range rects, each rendered as a full-figure-height
  rectangle styled with `vv-selection-range` CSS (parallel to
  `vv-brush-rect` — same translucent-fill + stroked-edges idiom).
  Multi-bp variants emit a single rect spanning `[anchorX, farX]`;
  SNVs degenerate to a zero-width rect, falling back to a single
  vertical line for visibility
- The viewer accepts the rects as a new internal prop computed from
  the active track's selected variants. No public API change — the
  rects are derived from `selectedFeatureIds` already on the public
  surface
- ClinVar opt-in to the same overlay: when a ClinVar mark is
  selected, the variant-track computes its range and emits the rect
  alongside any user-variant selection rects
- Detail card for user variants: same shape as ClinVar's
  `SelectedVariantCard`, minus significance / review / consequence
  rows. Shows the raw input, the canonical id, position, and the
  REF→ALT change

**Definition of done:**
- Clicking a user-variants mark draws the full-figure-height
  selection rect (or single line for SNVs); clicking elsewhere
  dismisses
- `?selected=<hash>` survives reload — the hashed id is restored as
  the selection
- A pre-hash `?selected=11-5226947-ACC…CCTGC-A` URL still resolves
  to the same variant (backward-compat path)
- Selecting a ClinVar mark draws the same rect over the ClinVar
  variant's range
- The selection rect lives in the existing brush-overlay layer, so
  it composes correctly with `<DefaultMinimap>` brush gestures and
  the box-zoom rect
- Playwright spec pins the selection round-trip, the hash backward
  compat, and the multi-bp-vs-SNV rendering difference
...
**Motivation:**
Slice 34's parser handles the common genomic forms but bails on
HGVS — `c.93G>A`, `p.Val123Met`, `g.74642513C>G`. Clinicians paste
HGVS more than VCF, and writing a local HGVS parser is a substantial
project (intron-offset syntax, the protein-coordinate three-letter
vs one-letter convention, the `delins` / `dup` / `inv` notations).
VariantValidator is the industry-standard service for exactly this
job: throw any HGVS or genomic flavour at it, get back the canonical
VCF representation.

**In scope:**
- Lazy fallback path: when the local parser fails OR the input
  starts with `c.` / `p.` / `g.` / `n.`, send the variant string
  (plus the transcript id) to VariantValidator's
  `/VariantValidator/variantvalidator/{genome_build}/{variant}/{transcripts}`
  endpoint
- Genome build assumed `GRCh38` for MVP (matches the playground's
  gnomAD source). No URL param to override
- In-memory `Map<rawInput, canonicalGenomicId>` cache scoped to the
  page session. Repeat URL loads with the same `?variants=…` hit
  the cache; navigating to a different transcript clears it
- Graceful failure: VV unreachable, rate-limited, or returning an
  error for a variant → the variant falls to the same footer
  parse-error path as Slice 34. Other variants still render. No
  retries, no proxy
- Concurrent variant resolution: parse each entry in parallel
  (`Promise.all`); the figure renders once all resolutions settle
  (or fail). Loading state shown via the existing shimmer overlay
  for the user-variants track row
- The VV call is gated to entries that didn't parse locally — a
  URL of all-canonical variants makes zero network calls

**Definition of done:**
- `?transcript=ENST00000269305&variants=c.524G>A,c.747G>T` renders
  the two missense variants on TP53 at the expected codon positions
- `?variants=p.Arg175His` round-trips correctly
- A URL with both HGVS (`c.524G>A`) and canonical
  (`17-7674212-C-T`) entries renders both, with the canonical one
  rendering without a network call
- VV down → the HGVS entries fall to the parse-error footer; the
  canonical entries still render
- Repeat load of the same `?variants=` URL hits the cache (no
  second network call); navigating to a different transcript
  clears it
- Unit tests mock VV responses for the happy / typo / unknown-tx
  cases; an e2e test pins the round-trip end-to-end against a
  mocked VV endpoint
...
**Motivation:**
Slices 34-36 make URLs work end-to-end, but constructing a URL by
hand isn't a clinical workflow — a user looking at a gene wants to
drop variants in without leaving the page. A keyboard-driven entry
modal closes the loop: hit `v`, paste / type, submit, see the
figure update.

The interaction is intentionally minimal — spotlight-style centered
card, focused textarea, submit on Cmd/Ctrl+Enter, Escape closes.
The actual variant parsing is the Slice 34 + 36 pipeline; this
slice is purely UX glue.

**In scope:**
- Global `v` keystroke listener on the embed view (skipped when an
  editable element has focus). Opens the modal; second press or
  Escape closes it
- Modal component: centered card, ~480px wide, ~3-line `<textarea>`
  (expands on overflow), submit / clear buttons, Cmd-Enter binding,
  click-outside-to-dismiss. Spotlight-shaped — minimal vertical
  chrome, big input
- Pre-population: the textarea opens with the current `?variants=`
  contents, one variant per line (the canonical form, not the raw
  input — the raw input is lost after Slice 34's normalisation,
  and that's fine for MVP)
- Input accepts both newline-separated AND comma-separated entries
  on submit. Submit canonicalises through the same Slice 34+36
  parser path
- Clear button empties the textarea only — figure and URL stay
  intact until submit (no destructive surprise)
- Submit rewrites `?variants=` via `history.replaceState`; the
  existing URL→state effect picks up the change and re-renders the
  track. Parse errors surface in the modal's footer (not the
  page's footer) until the modal closes, then the figure-level
  footer takes over
- Discoverability: a small `+` button in the toolbar opens the same
  modal with a `(V)` hint in its tooltip

**Definition of done:**
- Pressing `v` opens the modal pre-populated with the current
  variants; Escape closes it without changing anything
- Typing two new variants and Cmd-Entering updates the URL and the
  figure within a frame; the modal closes
- Click-outside closes without applying the input
- Parse errors render inline in the modal footer (red text); the
  user can fix the entry and re-submit without retyping the good
  ones
- Replace-all semantics: clearing the textarea then submitting an
  empty input removes the variants track entirely (URL drops
  `?variants=` query param)
- Playwright spec: open via hotkey, submit a list, assert URL +
  track update; open via toolbar `+`, clear, assert track removal
